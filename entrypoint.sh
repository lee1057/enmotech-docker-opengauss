...
docker_init_database_dir() {
        ...
        if [ -n "$GS_NODENAME" ]; then
                ...
                eval 'gs_initdb --pwfile=<(echo "$GS_PASSWORD") --nodename=mogdb --encoding=UTF-8 --locale=en_US.UTF-8 --dbcompatibility=PG -D $PGDATA'
        fi        
...
_main() {
        ...
        if [ "$1" = 'mogdb' ] && ! _opengauss_want_help "$@"; then
                docker_setup_env
